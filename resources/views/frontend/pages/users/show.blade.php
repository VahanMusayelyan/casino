        ...
        @if($user->id == auth()->user()->id || auth()->user()->role == "ADMIN")
            ...
            @if(session()->has('success'))
                <div class="alert alert-success alert-dismissible fade show" role="alert">
                    {{__('Your profile is successfully saved')}}
                </div>
            @endif
        @endif
    @endauth
...
{{--                <pie-chart :data="{{ json_encode($games_by_result) }}" theme="{{ $settings->theme }}" class="pie-chart"></pie-chart>--}}
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                    @foreach ($games_by_type->sortByDesc('value') as $game)
                        <tr>
                            <td>{{ $game['category'] }}</td>
                            <td class="text-right">{{ $game['value'] }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
...
{{--                </table>--}}
            @endif
        </div><div class="col-sm-12 col-lg-6 mt-3">
{{--            <div class="card border-primary text-center">--}}
{{--                <div class="card-body text-primary">--}}
{{--                    <h4 class="card-title">{{ __('Played :x games', ['x' => $total_played]) }}</h4>--}}
{{--                    <p class="card-text"><small class="text-muted">{{ __('Last played :t', ['t' => $last_played ? $last_played->diffForHumans() : __('never')]) }}</small></p>--}}
